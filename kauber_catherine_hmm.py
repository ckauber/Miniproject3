# -*- coding: utf-8 -*-
"""Kauber_Catherine_HMM.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1vqq7m567EA11QjsLFPrlFesnBbVXnd8n
...
class HiddenMarkovModel:
    ...
    def viterbi(self, x):
        ...
        #Find each hidden state's probability at start time
        ...
        for j in range(2, M+1):
          for i in range(self.L):
            ...
            for prev in range(self.L):
              temp_prob = probs[j-1][prev] * self.A[prev][i] * self.O[i][x[j-1]]
              temp_seq = seqs[j-1][prev] + str(i)
        ...
        max_prob = -100000000
        seq_idx = None
        for i in range(len(probs[M])):
          if probs[M][i] > max_prob:
            max_prob = probs[M][i]
            seq_idx = i
        max_seq = seqs[M][seq_idx]
        return max_seq
    ...
    def forward(self, x, normalize=False):
        ...
        #Again, initialize at start time 
        for i in range(self.L):
        ...
        for j in range(2, M+1):
          for i in range(self.L):
            for prev in range(self.L):
              alphas[j][i] += alphas[j-1][prev] * self.A[prev][i] * self.O[i][x[j-1]]
          if normalize == True:
            alpha_sum = sum(alphas[j])
            for alpha_idx, alpha in enumerate(alphas[j]):
              alphas[j][alpha_idx] = alpha / alpha_sum
    ...
    def backward(self, x, normalize=False):
        ...
        #initialize at end time bc we go backwards
        for i in range(self.L):
          betas[M][i] = 1

        #Now perform recursion backwards
        for j in range(M-1, 0, -1):
          for i in range(self.L):
            for prev in range(self.L):
              betas[j][i] += betas[j+1][prev] * self.A[i][prev]* self.O[prev][x[j]]
          if normalize == True:
            beta_sum = sum(betas[j])
            for beta_idx, beta in enumerate(betas[j]):
              betas[j][beta_idx] = beta / beta_sum
    ...
    def supervised_learning(self, X, Y):
        ...
        '''
        # Calculate A with M-step formulas
        L = self.L                      #The dimension of A is LxL
        A_ct = np.zeros((L,L)).tolist()

        for y in Y:
          for idx in range(len(y) - 1):
            A_ct[y[idx]][y[idx+1]] += 1

        for idx, row in enumerate(A_ct):
          row_sum = sum(row)
          self.A[idx] = [val / row_sum for val in row]


        # Calculate O with M-step formula
        L = self.L
        D = self.D                      #The dimension of O is LxD
        O_ct = np.zeros((L,D)).tolist()

        for x_idx, x in enumerate(X):
          y = Y[x_idx]
          for new_idx in range(len(y)):
            O_ct[y[new_idx]][x[new_idx]] += 1

        for idx, row in enumerate(O_ct):
          row_sum = sum(row)
          self.O[idx] = [val / row_sum for val in row]
    ...
    def unsupervised_learning(self, X, N_iters):
        ...
        for n in range(1, N_iters+1):
          print("iteration", n)
          observation_num = np.zeros((self.L, self.D)).tolist()
          observation_denom = np.zeros(self.L).tolist()
          
          transition_numer = np.zeros((self.L, self.L)).tolist()
          transition_denom = np.zeros(self.L).tolist()

          for x in X:
            alpha = self.forward(x, normalize = True)
            beta = self.backward(x, normalize = True)

            #Find Observation num and denom           
            for i in range(1, len(x)+1):
              probs = np.array([alpha[i][j] * beta[i][j] for j in range(self.L)])
              prob_sum = sum(probs)
              probs = [elem/prob_sum for elem in probs]

              for k in range(self.L):
                if i != len(x):
                  transition_denom[k] += probs[k]
                observation_num[k][x[i-1]] += probs[k]
                observation_denom[k] += probs[k]

            #Find transition num 
            for i in range(1, len(x)):
              probs_A = np.array([[alpha[i][j] * beta[i+1][k] * self.A[j][k] * 
                                   self.O[k][x[i]] for k in range(self.L)] for j in range(self.L)])
              trans_sum = 0
              for prob in probs_A:
                trans_sum += sum(prob)
              ...
              for j in range(self.L):
                for k in range(self.L):
                  probs_A[j][k] = probs_A[j][k]/trans_sum
                  transition_numer[j][k] += probs_A[j][k]

          
          #Now we have numerators and denominators. Just have to divide
          self.A = [[transition_numer[j][k]/transition_denom[j] for k in range(self.L)] for j in range(self.L)]
          self.O = [[observation_num[j][k]/observation_denom[j] for k in range(self.D)] for j in range(self.L)]
    ...
    def generate_emission(self, M, seed=None):
        ...
        start_state = np.argmax(rng.multinomial(1, self.A_start))
        for i in range(M):
          states.append(start_state)
          obs_prob = self.O[start_state]
          rand = rng.uniform(0, 1)
          observ = np.searchsorted(np.cumsum(obs_prob), rand)
          emission.append(observ)
          
          trans_prob = self.A[start_state]
          rand = rng.uniform(0, 1)
          transit = np.searchsorted(np.cumsum(trans_prob), rand)
          start_state = transit
...
unsupervised_learning(4, 0, seed=1)
